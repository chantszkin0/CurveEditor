...
Matrix::Matrix(const double* array, int rows, int cols) : rows_(rows), cols_(cols)
{
    allocSpace();
    for(int i = 0; i < rows_; ++i){
        for(int j = 0; j < cols_; ++j){
            p[i][j] = array[i*cols_+j];
        }
    }
}

Matrix::Matrix(const Vector3& a, const Vector3& b, const Vector3& c, const Vector3& d) : rows_(3), cols_(4)
{
    allocSpace();
    for(int i = 0; i < rows_; ++i){
        p[i][0] = a[i];
...
Matrix::Matrix(std::string name, double power)
{
    if(name == "CubicBezier"){
        rows_ = 4; cols_ = 4;
        allocSpace();
        int data[4][4] = {{1, -3, 3, -1},{0, 3, -6, 3},{0, 0, 3, -3},{0, 0, 0, 1}};
        for(int i = 0; i < rows_; ++i)
            for(int j = 0; j < cols_; ++j)
                p[i][j] = data[i][j];
    }
    else if(name == "PowerBasis"){
        rows_ = 4; cols_ = 1;
        allocSpace();
        double data[4][1] = {{1}, {power}, {power*power}, {power*power*power}};
        for(int i = 0; i < rows_; ++i)
            for(int j = 0; j < cols_; ++j)
                p[i][j] = data[i][j];
    }
...
Vector3 Matrix::getVector(int i) const
{
    Vector3 temp = {p[0][i], p[1][i], p[2][i]};
    return temp;
}
